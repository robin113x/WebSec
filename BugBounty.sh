...
APIKEY="$SHODAN_API_KEY"  # From environment variable
WORDLISTS="/usr/share/wordlists/subdomain/sub.txt" # Or your preferred wordlist path
FUZZ_WORDLIST="/usr/share/wordlists/dirb/common.txt" # Wordlist for fuzzing
EXTENSIONS="xml,json,sql,db,log,yml,yaml,bak,txt,tar.gz,zip,php,aspx,jsp,html" # Extensions for dirsearch
DNSCAN_WORDLIST="/usr/share/wordlists/subdomain/sub.txt" # Wordlist specifically for dnscan
